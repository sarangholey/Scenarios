...
public abstract class Interact {
	...
	public WebElement clearElementText(By by) {
		WebDriverWait wait = new WebDriverWait(driver, 60);
		WebElement element = wait.until(ExpectedConditions.elementToBeClickable(by));
		element.clear();
		logger.info("Element Text is Cleared");
		return element;
	}
	...
	public WebDriver switchToFrame(By by) {
		WebDriver value=driver.switchTo().frame(getDriver().findElement(by));
